...
export default function ServicesGrid() {
  ...
  // Helper to display price correctly
  ...
  const isFreeQuote = (price: string) => price.toLowerCase() === "free quote";
    ...
    >
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-20 gap-y-24">
          {services.map((item) => {
            const freeQuote = isFreeQuote(item.price);
            return (
              <div key={item.id} className="group relative flex flex-col h-[700px]">
                ...
                {/* Price Seal - perfectly centered vertically */}
                <div className="absolute top-1/2 -translate-y-1/2 right-6 z-30 transform group-hover:scale-110 group-hover:-translate-y-[calc(50%+0.5rem)] transition-all duration-500">
                  <div className={`rounded-full bg-[#F2CF51] flex flex-col items-center justify-center border-8 border-white shadow-[0_20px_40px_rgba(242,207,81,0.3)] ${
                    freeQuote ? "w-24 h-24" : "w-28 h-28"
                  }`}>
                    {!freeQuote && (
                    ...
                    <span className={`font-black text-[#12416B] italic text-center leading-tight ${
                      freeQuote ? "text-base px-2" : "text-2xl"
                    }`}>
